...
contract Trade {
  ...
  constructor(SeedBlock _tree, address _seedToken) public {
    tree = _tree;
    SEED_CONTRACT = ISeedToken(_seedToken);
    _manager = tree.getOwner();
  }
  
  /** @dev List SeedBlock Forest NFT for sell at specific price 
  ...
  function listTree(uint256 _id, uint256 _price, uint256 _burnPercent) public {
    ...
    require(
      ...
      " SeedBlock: Contract is not approved to manage token of this ID "
    );

    require(
      msg.sender == _owner,
      " SeedBlock: Only owner of token can list the token for sell "
    );
  ...
  /** @dev Buy SeedBlock Forest NFT for listed price 
  ...
  function buyTree(uint256 _id, uint256 _value) public {
     ...
     require(
      ...
      " SeedBlock: Token of this ID is not for sell "
      );
     
     require(
      treeList[_id].price <= _value,
      " SeedBlock: Provided value is less than listed price "
      );
